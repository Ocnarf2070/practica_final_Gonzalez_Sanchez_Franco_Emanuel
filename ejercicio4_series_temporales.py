...
def analizar_residuo(residuo):
    ...
    plot_acf(residuo_limpio, ax=ax[0])
    plot_pacf(residuo_limpio, ax=ax[1])
    plt.tight_layout()
    ...
    plt.figure()
    plt.hist(residuo_limpio, bins=11, density=True)
    x=np.linspace(min(residuo_limpio), max(residuo_limpio),100)
    ...
    with open("output/ej4_analisis.txt", "w", encoding="utf-8") as f:
        f.write("Análisis estadístico para el residuo\n")
        f.write("=" * 50 + "\n")
        f.write(f"  Media  : {media:.2f}\n")
        f.write(f"  Desviación Estándar : {std:.2f}\n")
        str_asimetria=""
        if asimetria == 0: str_asimetria = "Simetría"
        elif asimetria > 0: str_asimetria = "Asimetría positiva"
        else: str_asimetria = "Asimetría negativa"
        f.write(f"  Asimetría   : {asimetria:.2f} => {str_asimetria}\n")
        str_curtosis=""
        if curtosis == 0: str_curtosis = "Curva Mesocúrtica"
        elif curtosis > 0: str_curtosis = "Curva Leptocúrtica"
        else: str_curtosis = "Curva Platicúrtica"
        f.write(f"  Curtosis   : {curtosis:.2f} => {str_curtosis}\n")
        f.write("-" * 50 + "\n")
        f.write(f"  Test de normalidad Shapiro-Wilk o Jarque-Bera (p-value)  : {p:.2f} => {'Se acepta normalidad' if p > 0.05 else 'Se rechaza normalidad'}\n")
        f.write(f"  ADF Test para verificar estacionariedad (p-value)  : {p_adf:.2f} => {'Se acepta estacionariedad' if p_adf < 0.05 else 'Se rechaza estacionariedad'}\n")
