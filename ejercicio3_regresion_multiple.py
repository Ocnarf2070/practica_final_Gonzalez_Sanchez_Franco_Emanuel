...
def regresion_lineal_multiple(X_train, y_train, X_test):
    ...
    n = X_train.shape[0]
    ones = np.ones((n,1))
    X_train_b = np.hstack([ones, X_train])  # ← Reemplaza None con tu implementación
...
def calcular_rmse(y_real, y_pred):
    ...
    n = len(y_real)
    y_diff = np.pow(y_real - y_pred,2)
    return np.sqrt(sum(y_diff)/n)
...
def calcular_r2(y_real, y_pred):
    ...
    y_diff_res = np.pow(y_real - y_pred,2)
    y_mean = np.mean(y_real)
    y_diff_tot = np.pow(y_real - y_mean,2)
    SS_res = sum(y_diff_res)
    SS_tot = sum(y_diff_tot)
...
def graficar_real_vs_predicho(y_real, y_pred, ruta_salida="output/ej3_predicciones.png"):
    ...
    # Para crear la linea, se ve cuales son los limites de la gráfica de puntos
    xmin, xmax, ymin, ymax = plt.axis()

    # Después obtenemos entre los valores mínimos y máximos de x e y, se ve cual es el mínimo y el máximo entre x e y
    min_limit = min(xmin,ymin)
    max_limit = max(xmax,ymax)

    # Y asi dando lugar a los puntos para el mínimo valor y el máximo valor, el cual cumple que sea y=x
    plt.plot([min_limit,max_limit],[min_limit,max_limit],c='r')

    plt.legend(['Puntos','Linea perfecta'],loc="lower right")
